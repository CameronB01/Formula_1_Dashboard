...
drivers$full_name <-
  paste(drivers$forename, drivers$surname, sep = " ")
drivers <- drivers %>%
  ...
  dashboardBody(
    tabItems(tabItem(
      ...
      fluidRow(
        box(
          ...
          actionButton(
            "more_info_button1",
            label = icon("info-circle", lib = "font-awesome"),
            style = "padding: 3px; background-color: #272c30; color: white; border-color: #272c30; float: right;"
          )
        ),
        ...
        box(
          ...
          selectInput(
            "name",
            "Select a Driver:",
            choices = unique(unique_sorted_drivers$driverRef),
            selected = "Lewis Hamilton"
          ),
          div(style = "display: flex; flex-direction: column; align-items: center; margin-bottom: 20px; font-size: 15px",
        ...
        box(
          ...
          selectInput(
            "table_year",
            "Select a year:",
            choices = sort(unique(result_data$year), decreasing = TRUE)
          ),
        ...
        box(
          ...
          fluidRow(column(width = 5, uiOutput(
            "raceYearSelection"
          )),
          column(
            width = 7, uiOutput("raceCircuitSelection")
          )),
        ...
        box(
          ...
          selectInput(
            "laps_heatmap_year",
            "Select a year:",
            choices = sort(unique(standings_driver$year)[unique(standings_driver$year) > 1995], decreasing = TRUE)
          ),
        ...
        box(
          ...
          fluidRow(
            column(
              width = 2,
              checkboxInput("race_seconds_checkbox", "Driver Comparison", value = FALSE)
            ),
            column(
              width = 3,
              checkboxInput("out_rem", "Remove Pit Stops and Race Interruptions", value = FALSE)
            )
          ),
            ...
            fluidRow(
              box(
                ...
                fluidRow(column(
                  ...
                  selectInput(
                    "end_year",
                    "Select a year:",
                    choices = sort(unique(constructor_data$year), decreasing = TRUE)
                  )
                ),
              ...
              box(
                ...
                selectInput(
                  "constructor_team",
                  "Select a team:",
                  choices = unique(constructor_leaderboard$team)
                ),
              ...
              box(
                width = 12,
                title = "Lap Positions by Constructor",
                fluidRow(
                  column(width = 4, uiOutput("raceYearSelection_driver_position")),
            ...
            fluidRow(
              box(
                ...
                selectInput(
                  "race_year",
                  "Select a starting year:",
                  choices = sort(unique(total_active_races$Year), decreasing = TRUE)
                ),
                leafletOutput("world_plot", width = "100%", height = "68vh"),
                actionButton(
                  "more_info_button8",
                  label = icon("info-circle", lib = "font-awesome"),
                  style = "padding: 3px; background-color: #272c30; color: white; border-color: #272c30; float: right;"
              ...
              box(
                ...
                selectInput(
                  "circuit_name",
                  "Select a Circuit:",
                  choices = unique(total_active_races[total_active_races$Year == 2023,]$Circuit)
                ),
                imageOutput("circuit_img", height = "35em")
              ),
            )), 
    tabItem(tabName = "car_plots",
            fluidRow(
              ...
              box(
                width = 12,
                title = "Number of World Championships by Driver",
                plotOutput("world_champtionships", height = "65vh"),
                actionButton(
                  "more_info_button21",
                  label = icon("info-circle", lib = "font-awesome"),
                  style = "padding: 3px; background-color: #272c30; color: white; border-color: #272c30; float: right;"
              ...
              box(
                ...
                selectInput(
                  "box_circuits",
                  "Select a circuit:",
                  choices = sort(unique(result_data_comp[complete.cases(result_data_comp$race_fastestLapSpeed),]$Circuit), decreasing = FALSE)
                ),
              ...
              box(
                ...
                selectInput(
                  "pit_circuit",
                  "Select a circuit:",
                  choices = sort(unique(pit_times[complete.cases(pit_times$pit_stop_duration),]$Circuit), decreasing = FALSE)
                ),
            ...
            fluidRow(
              box(
                ...
                selectInput(
                  ...
                  "Enter a drivers name:",
                  choices = unique(drivers$driverRef)
                ),
              ...
              box(
                ...
                fluidRow(column(
                  width = 6,
                  selectInput(
                    "accident_constructor_year",
                    "Select a year:",
                    choices = sort(unique(result_data$year), decreasing = TRUE)
                  )
                ),
                column(
                  width = 6, uiOutput("constructor_retirement_team")
                )),
...
server <- function(input, output) {
  shinyalert(
    "Welcome to F1 Insights:",
    "Your one-stop shop for comprehensive Formula 1 data and analysis. Whether you're a seasoned fan or a curious newcomer, F1 Insight provides you with the tools to delve into the world of Formula 1 and gain a deeper understanding of the sport. Explore detailed statistics on your favorite teams and drivers, analyze race results and performance trends, and uncover insights into circuit characteristics.",
    type = "info"
  )
  ...
  observeEvent(input$more_info_button2, {
    showModal(modalDialog(title = tags$div(
      HTML(
        "<span style='color: black;'>Driver's Team Timeline</span>"
      )
    ),
    tags$div(
      HTML(
  ...
  observeEvent(input$more_info_button3, {
    showModal(modalDialog(title = tags$div(
      HTML("<span style='color: black;'>Race Finish History</span>")
    ),
    tags$div(
      HTML(
  ...
  observeEvent(input$more_info_button4, {
    showModal(modalDialog(title = tags$div(
      HTML(
        "<span style='color: black;'>Driver Race Position History</span>"
      )
    ),
    tags$div(
      HTML(
  ...
  observeEvent(input$more_info_button5, {
    showModal(modalDialog(title = tags$div(
      HTML(
        "<span style='color: black;'>Constructor Points Through the Years</span>"
      )
    ),
    tags$div(
      HTML(
  ...
  observeEvent(input$more_info_button6, {
    showModal(modalDialog(title = tags$div(
      HTML(
        "<span style='color: black;'>Constructor Final Ordering</span>"
      )
    ),
    tags$div(
      HTML(
  ...
  observeEvent(input$more_info_button7, {
    showModal(modalDialog(title = tags$div(
      HTML(
        "<span style='color: black;'>Historical Constructor Performance</span>"
      )
    ),
    tags$div(
      HTML(
  ...
  observeEvent(input$more_info_button8, {
    showModal(modalDialog(title = tags$div(
      HTML("<span style='color: black;'>Track Locations</span>")
    ),
    tags$div(
      HTML(
  ...
  observeEvent(input$more_info_button9, {
    showModal(modalDialog(title = tags$div(
      HTML("<span style='color: black;'>Lap Positions</span>")
    ),
    tags$div(
      HTML(
  ...
  observeEvent(input$more_info_button10, {
    showModal(modalDialog(title = tags$div(
      HTML(
        "<span style='color: black;'>Laps Spent in Each Position</span>"
      )
    ),
    tags$div(
      HTML(
  ...
  observeEvent(input$more_info_button11, {
    showModal(modalDialog(title = tags$div(
      HTML("<span style='color: black;'>Top Speeds</span>")
    ),
    tags$div(
      HTML(
  ...
  observeEvent(input$more_info_button12, {
    showModal(modalDialog(title = tags$div(
      HTML("<span style='color: black;'>Pit Stop Time</span>")
    ),
    tags$div(
      HTML(
  ...
  observeEvent(input$more_info_button13, {
    showModal(modalDialog(title = tags$div(
      HTML("<span style='color: black;'>Driver Retirements</span>")
    ),
    tags$div(
      HTML(
  ...
  observeEvent(input$more_info_button14, {
    showModal(modalDialog(title = tags$div(
      HTML("<span style='color: black;'>Fatal Crashes</span>")
    ),
    tags$div(
      HTML(
  ...
  observeEvent(input$more_info_button15, {
    showModal(modalDialog(title = tags$div(
      HTML(
        "<span style='color: black;'>Constructor Retirements</span>"
      )
    ),
    tags$div(
      HTML(
  ...
  observeEvent(input$more_info_button16, {
    showModal(modalDialog(title = tags$div(
      HTML(
        "<span style='color: black;'>Safety Car Deployments</span>"
      )
    ),
    tags$div(
      HTML(
  ...
  observeEvent(input$more_info_button17, {
    showModal(modalDialog(title = tags$div(
      HTML(
        "<span style='color: black;'>Average Driver Age by Year</span>"
      )
    ),
    tags$div(
      HTML(
  ...
  observeEvent(input$more_info_button18, {
    showModal(modalDialog(title = tags$div(
      HTML(
        "<span style='color: black;'>Championship Winning Driver Age</span>"
      )
    ),
    tags$div(
      HTML(
  ...
  observeEvent(input$more_info_button19, {
    showModal(modalDialog(title = tags$div(
      HTML("<span style='color: black;'>Driver Career Wins</span>")
    ),
    tags$div(
      HTML(
  ...
  observeEvent(input$more_info_button20, {
    showModal(modalDialog(title = tags$div(
      HTML(
        "<span style='color: black;'>Lap Time and Pit Stop Trends</span>"
      )
    ),
    tags$div(
      HTML(
  ...
  observeEvent(input$more_info_button21, {
    showModal(modalDialog(title = tags$div(
      HTML(
        "<span style='color: black;'>Lap Time and Pit Stop Trends</span>"
      )
    ...
    tags$div(
      HTML(
        "<span style='color: black;'>This bar chart presents a comparison of the number of world championships won by various drivers. Each bar represents a specific driver, positioned along the y-axis, while the x-axis indicates the number of world championships won. This visual representation offers a clear overview of the dominance or success of different drivers in the history of world championships, enabling a quick comparison of their respective accomplishments.</span>"
      )
    )))
  })
  ...
  output$raceYearSelection_driver_position <- renderUI({
    position_years <- race_data %>%
      ...
      arrange(desc(year)) %>%
      pull(year)
    selectInput("driver_race_year_driver_position",
                "Select a year:",
                choices = position_years)
  })
  ...
  output$raceCircuitSelection_driver_position <- renderUI({
    position_drive_circuit <- race_data %>%
      ...
      select(Circuit) %>%
      unique()
    selectInput("driver_race_circuit_driver_position",
                "Select a Circuit:",
                choices = position_drive_circuit)
  })
  ...
  output$raceCircuitSelection_team <- renderUI({
    ...
    race_data_position_teams = race_data %>%
      filter(
        year == input$driver_race_year_driver_position,
        Circuit == input$driver_race_circuit_driver_position
      ) %>%
    ...
    selectInput("team_color",
                "Choose a team:",
                choices = unique(race_data_position_teams$Team))
  })
  ...
  output$teamSelection <- renderUI({
    active_teams <- constructor_data %>%
      filter(year >= as.numeric(input$end_year) - 1,
             year <= input$end_year) %>%
      group_by(team) %>%
      filter(n_distinct(year) == 2) %>%
      ungroup()
    
    selectInput("active_teams", "Select a team:", choices = active_teams$team, selected = "Mclaren")
  })
  ...
  output$constructor_retirement_team <- renderUI({
    ...
    selectInput(
      "accident_constructor_name",
      "Enter a constructors name:",
      choices = unique(retirement_teams$team)
    )
  })
  ...
  output$raceSecondsCircuit <- renderUI({
    ...
    selectInput(
      "race_seconds_circuit",
      "Select a circuit:",
      choices = unique(time_seconds_year$Circuit)
    )
  })
  ...
  output$raceSecondsDriver1 <- renderUI({
    time_seconds_circuit1 <- race_data %>%
      filter(year == input$race_seconds_year,
             Circuit == input$race_seconds_circuit) %>%
      select(driverRef) %>%
      unique()
    
    selectInput(
      "race_seconds_driver1",
      "Select a driver:",
      choices = unique(time_seconds_circuit1$driverRef),
      selected = "Max Verstappen"
    )
  })
  ...
  output$raceSecondsDriver2 <- renderUI({
    ...
    selectInput(
      "race_seconds_driver2",
      "Select driver to compare:",
      choices = unique(time_seconds_circuit2$driverRef),
      selected = "Lewis Hamilton"
    )
  })
      ...
      fluidRow(
        column(
          ...
          selectInput(
            "race_seconds_year",
            "Select a year:",
            choices = sort(unique(race_data$year), decreasing = TRUE)
          )
        ),
  ...
  output$line_plot <- renderPlot({
    if (input$line_plot_checkbox) {
      ...
      ggplot(
        ...
        geom_line(
          data = filtered_data_line_plot() %>% filter(year == input$driver_year),
          aes(color = "red"),
          size = 1
        ) +
        labs(x = "Round",
  ...
  output$slope_plot <- renderPlot({
    ...
    ggplot(filtered_data_slope_plot(),
           aes(
             x = year,
             y = max_value,
             group = team,
      ...
      geom_line(
        data = filtered_data_slope_plot() %>% filter(team == input$active_teams),
        color = "red",
        size = 1
      ) +
      geom_point(
  ...
  output$circuit_img <- renderImage({
    circuit_imgs = paste("active_circuits/", unique(total_active_races$Circuit[total_active_races$Circuit == input$circuit_name]), ".jpg", sep = "")
  ...
  filtered_data_race_position <- reactive({
    subset(
      race_data,
      driverRef == input$name &
        year == input$driver_race_year &
        Circuit == input$driver_race_circuit
    )
  })
  ...
  output$race_grid_position <- renderPlot({
    if (nrow(filtered_data_race_position()) == 0) {
      ...
      invis <- data.frame(lap = c(1), 
                          lap_times_position = c(1))  
      
      ggplot(filtered_data_race_position(),
             aes(lap, lap_times_position)) +
        ...
        scale_y_continuous(breaks = seq(
          1,
          max(filtered_data_race_position()$lap_times_position),
          by = 1
        ), trans = "reverse")
    }
  })
      ...
      output$final_position <- renderPlot({
        ...
        if (nrow(driver_filter) == 0) {
          ...
          ggplot(counter, aes(race_position, n)) +
            ...
            scale_x_reverse(breaks = seq(1, max(
              na.omit(driver_filter$race_position)
            ), by = 1)) +
            theme_minimal() +
            theme(
              panel.grid.minor = element_blank(),
            ...
            scale_y_continuous(
              breaks = seq(0, max(na.omit(counter$n)) + 4, by = 10),
              limits = c(0, max(na.omit(counter$n)) + 4),
              na.value = NA
            )
        }
      })
      ...
      output$teams_driver <- renderPlot({
        ...
        ggplot(driver, aes(
          ...
          scale_y_continuous(
            breaks = seq(0, max(na.omit(driver$n)) + 1, by = 10),
            limits = c(0, max(na.omit(driver$n)) + 1),
            na.value = NA
          )
      })
      ...
      output$constructor_performance <- renderPlot({
        ggplot(filtered_constructor(),
               aes(
                 x = reorder_within(team, max_value, year),
                 y = max_value,
                 fill = team
               )) +
          geom_bar(stat = "identity") +
      ...
      output$constructor_ranking <- renderPlotly({
        ...
        const_plot <- ggplot(constructor_leaderboard, aes(Year, Rank)) +
          ...
          geom_point(data = filtered_constructor_ranking(),
                     aes(text = tooltip),
                     color = "red") +
          theme_minimal() +
          theme(
      ...
      output$every_position <- renderPlotly({
        ...
        pos_plot = ggplot(race_data_position, aes(Lap, Position, group = Driver)) +
          ...
          geom_line(
            data = race_data_position %>% filter(Team == input$team_color),
            aes(text = paste("Team:", Team)),
            color = "red"
          ) +
          labs(x = "Lap", y = "Position") +
          theme_minimal() +
      ...
      output$speed_box <- renderPlot({
        ...
        race_2004 <-
          race_2004[complete.cases(race_2004$race_fastestLapSpeed),]
        ...
        race_2004_filtered <- race_2004 %>%
          filter(race_fastestLapSpeed >= lower_bound &
                   race_fastestLapSpeed <= upper_bound)
        
        
        
        ggplot(race_2004_filtered,
               aes(x = factor(year), y = race_fastestLapSpeed)) +
      ...
      output$pit_box <- renderPlot({
        ...
        pit_times_filtered <- pit_times_circuit %>%
          filter(pit_stop_duration >= lower_bound &
                   pit_stop_duration <= upper_bound)
      ...
      output$driver_age <- renderPlot({
        ...
        ggplot(na.omit(average_by_year), aes(year, avg_value)) +
          ...
          scale_x_continuous(breaks = seq(
            min(average_by_year$year),
            max(average_by_year$year),
            by = 10
          ), na.value = NA) +
          scale_y_continuous(breaks = seq(round(min(
            average_by_year$avg_value
          )),
          round(max(
            average_by_year$avg_value
          )),
      ...
      output$winners_age <- renderPlot({
        ...
        max_points_per_year$age_at_win <-
          interval(max_points_per_year$dob, max_points_per_year$race_date) %/% years(1)
        ...
        ggplot(max_points_per_year, aes(year, age_at_win)) +
          ...
          scale_x_continuous(breaks = seq(
            min(max_points_per_year$year),
            max(max_points_per_year$year),
            by = 10
          ), na.value = NA) +
          scale_y_continuous(
      ...
      output$driver_team_win <- renderPlot({
        ...
        if (nrow(driver_team_wins) == 0) {
          ggplot() +
            geom_text(
              aes(x = 0.5, y = 0.5, label = "This driver does not have any wins yet"),
              color = "white",
              size = 15,
              ha = "center",
              va = "center"
          ...
          ggplot(driver_team_wins, aes(year, n, fill = team)) +
            ...
            scale_x_continuous(breaks = seq(
              min(driver_team_wins$year),
              max(driver_team_wins$year),
              by = 1
            ),
            na.value = NA) +
            scale_y_continuous(
              breaks = seq(0, max(driver_team_wins$n) + 1, by = 5),
              limits = c(0, max(driver_team_wins$n) + 1),
              na.value = NA
            )
      ...
      output$constructor_table <- renderDataTable({
        ...
        datatable(
          table_constructor_result_standings,
          options = list(
            scrollY = "400px",
            paging = FALSE,
            initComplete = JS(
          ...
          DT::formatStyle(
            columns = names(table_constructor_result_standings),
            color = "white"
          )
      })
      ...
      output$constructor_accidents <- renderPlot({
        ...
        status_constructor_results = status_constructor_results %>%
          ...
          left_join(constructors, by = "constructorId") %>%
          filter(
            team == input$accident_constructor_name,
            year == input$accident_constructor_year
          ) %>%
          count(status) %>%
          arrange(desc(n))
      ...
      output$fatal_car_accidents <- renderPlot({
        ...
        ggplot(fatal_accidents_count, aes(reorder(!!sym(
          input$accident_type
        ), n), n)) +
          geom_bar(stat = "identity", fill = "#CBCBCC") +
          coord_flip() +
      ...
      output$laps_heatmap <- renderPlot({
        ...
        long_race_laps$driverRef <-
          factor(long_race_laps$driverRef,
                 levels = rev(driver_order$driverRef))
        
        
        ggplot(long_race_laps,
               aes(
                 lap_times_position,
                 driverRef,
                 fill = n,
      ...
      output$race_seconds <- renderPlot({
        if (input$race_seconds_checkbox) {
          seconds_data_driver_1 <- race_data %>%
            ...
            filter(
              year == input$race_seconds_year,
              Circuit == input$race_seconds_circuit,
              driverRef == input$race_seconds_driver1
            )
          ...
          seconds_data_driver_2 <- race_data %>%
            ...
            filter(
              year == input$race_seconds_year,
              Circuit == input$race_seconds_circuit,
              driverRef == input$race_seconds_driver2
            )
          ...
          remove_outliers <- function(seconds_data) {
            ...
            outliers <-
              seconds_data$lap_time_seconds < lower_bound |
              seconds_data$lap_time_seconds > upper_bound
          ...
          cleaned_data_driver_1 <-
            remove_outliers(seconds_data_driver_1)
          cleaned_data_driver_2 <-
            remove_outliers(seconds_data_driver_2)
          
          if (input$out_rem) {
            combined_drivers <-
              rbind(cleaned_data_driver_1, cleaned_data_driver_2)
          } else {
            combined_drivers <-
              rbind(seconds_data_driver_1, seconds_data_driver_2)
          }
          
          ggplot(combined_drivers,
                 aes(lap, lap_time_seconds, color = driverRef)) +
            geom_line(size = 1) +
      ...
      output$world_champtionships <- renderPlot({
        championships = standings_driver%>%
          group_by(year)%>%
          filter(driver_points == max(driver_points))%>%
        ...
        ggplot(championships, aes(reorder(driverRef, n), n))+
          geom_bar(stat = "identity", fill = "#CBCBCC")+
          ...
          scale_y_continuous(breaks = seq(0, max(championships$n), by = 1))
      })
